...
/**
 * Comando para mostrar todos los registros de la tabla: <xsl:value-of select="$tabla"/>
...
<!-- 
	Optiene todos los field de la tabla, por cada campo en la tabla  
	Adiciona una condicion para llamar a la clase que llena la session.
--> 
<xsl:template mode="CamposTabla" match="field">
       if(!WebSession::issetProperty("<xsl:value-of select="ancestor::table/name"/>__<xsl:value-of select="name"/>"))
           WebSession::setProperty("<xsl:value-of select="ancestor::table/name"/>__<xsl:value-of select="name"/>",$<xsl:value-of select="ancestor::table/name"/>__<xsl:value-of select="name"/>);
</xsl:template>
