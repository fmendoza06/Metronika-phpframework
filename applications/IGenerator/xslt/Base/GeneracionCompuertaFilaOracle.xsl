 ...
 */
class <xsl:value-of select="$empresa"/><xsl:value-of select="$aplicacion"/>Oracle<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="name"/></xsl:call-template>
{
	/**
...
<xsl:template match="declaration">
	...
	* @author SpyroFrameWork
	*/
  	function <xsl:value-of select="$empresa"/><xsl:value-of select="$aplicacion"/>Oracle<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="ancestor::table/name"/></xsl:call-template>()
  	{
	...
	*/    
  	function exist<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="ancestor::table/name"/></xsl:call-template>(<xsl:apply-templates mode="LLAVES_PRIMARY_PARAMETROS" select="primary_key/field"/>)
  	{
    	...
    	return OCIFetchStatement($this->consult,$rows);
  	}
	...
	*/   
  	...
  	{   
    	...
    	$this->consult = OCIParse($this->connection,$sql);
    	return @OCIExecute($this->consult);
  	}
  ...
  <xsl:apply-templates mode="FUNCIONES_GET" select="ancestor::table/declaration/field"/>
	/**
	* Metodo para convertir un recurso de base de datos a un vector asociativo 
	* @author SpyroFrameWork
	* @return array
	*/
...
<xsl:template mode="PRIMERA_FOREIGN_KEY" match="declaration/field">
 <xsl:param name="nameFK"/>
  <xsl:if test="name = $nameFK">
         <xsl:if test="type = 'varchar' or type = 'varchar2' or type = 'date' or type = 'char' or type = 'long' or type = 'raw'">\"<xsl:value-of select="name"/>\"='$<xsl:value-of select="name"/>'</xsl:if>
	     <xsl:if test="type = 'float' or type = 'number'">\"<xsl:value-of select="name"/>\"=$<xsl:value-of select="name"/></xsl:if>
  </xsl:if>
</xsl:template>
...
<xsl:template mode="DEMAS_FOREIGN_KEY" match="declaration/field">
 <xsl:param name="nameFK"/>
  <xsl:if test="name = $nameFK">
	     <xsl:if test="type = 'varchar' or type = 'varchar2' or type = 'date' or type = 'char' or type = 'long' or type = 'raw'"> AND \"<xsl:value-of select="name"/>\"='$<xsl:value-of select="name"/>'</xsl:if>
	     <xsl:if test="type = 'float' or type = 'number'"> AND \"<xsl:value-of select="name"/>\"=$<xsl:value-of select="name"/></xsl:if>
  </xsl:if>
</xsl:template>
<!-- ************************************************************************************************************ -->
	...
	<xsl:variable name="cadena"><!-- toma la cadena de los fields con comas -->
	 <xsl:for-each select="field">
	   <xsl:if test="not(ancestor::declaration/primary_key/field/name = name)">
	     <xsl:if test="type = 'varchar' or type = 'varchar2' or type = 'char' or type = 'long' or type = 'raw'">\"<xsl:value-of select="name"/>\"='$<xsl:value-of select="name"/>',</xsl:if>  
...
<!-- ************************************************************************************************************ -->
  ...
  <xsl:if test="position() = 1">  
    <xsl:if test="$tipo_dato = 'varchar' or type = 'varchar2' or $tipo_dato = 'date' or $tipo_dato = 'char' or $tipo_dato = 'long' or type = 'raw'">\"<xsl:value-of select="name"/>\"='$<xsl:value-of select="name"/>'</xsl:if>
    <xsl:if test="$tipo_dato = 'float' or $tipo_dato = 'number'">\"<xsl:value-of select="name"/>\"=$<xsl:value-of select="name"/></xsl:if>
  </xsl:if>

  <xsl:if test="position() != 1">     
	<xsl:if test="$tipo_dato = 'varchar' or type = 'varchar2' or $tipo_dato = 'date' or $tipo_dato = 'char' or $tipo_dato = 'long' or type = 'raw'"> AND \"<xsl:value-of select="name"/>\"='$<xsl:value-of select="name"/>'</xsl:if>
	<xsl:if test="$tipo_dato = 'float' or $tipo_dato = 'number'"> AND \"<xsl:value-of select="name"/>\"=$<xsl:value-of select="name"/></xsl:if>
  </xsl:if>
...
<xsl:template mode="PARAMETROS_INSERT" match="field"> <!--PARM_SQL-->
     <xsl:choose>
			<xsl:when test="position() = 1">
 				  <xsl:if test="type = 'varchar' or type = 'varchar2' or type = 'char' or type = 'long' or type = 'raw'">'$<xsl:value-of select="name"/>'</xsl:if>
				  <xsl:if test="type = 'date'">TO_DATE('$<xsl:value-of select="name"/>','YYYY/MM/DD')</xsl:if> 
				  <xsl:if test="(type = 'float' or type = 'number') and not(counter)">$<xsl:value-of select="name"/></xsl:if>
...
<metaInformation>
<scenarios ><scenario default="no" name="Producto" userelativepaths="yes" externalpreview="no" url="metadata_oracle.xml" htmlbaseurl="" outputurl="" processortype="msxml" profilemode="0" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" ><parameterValue name="tabla" value="'emp'"/></scenario><scenario default="yes" name="oracle" userelativepaths="yes" externalpreview="no" url="file://d:\My Documents\Tesis\Metadatos\dbpruebamysql2.xml" htmlbaseurl="" outputurl="" processortype="internal" profilemode="0" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" ><parameterValue name="tabla" value="'ciudad'"/><parameterValue name="empresa" value="'Gs'"/><parameterValue name="aplicacion" value="'Seg'"/></scenario></scenarios><MapperInfo srcSchemaPath="" srcSchemaRoot="" srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/>
</metaInformation>
-->