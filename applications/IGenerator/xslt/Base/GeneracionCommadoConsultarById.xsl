...
/**
 * Comando para consultar un registro de la tabla: <xsl:value-of select="$tabla"/>
...
<xsl:template match="declaration">
	...
	function execute()
    {
        ...
        if(<xsl:apply-templates select="primary_key"/>){
           ...
           $<xsl:value-of select="ancestor::table/name"/>_data = $<xsl:value-of select="ancestor::table/name" />_manager->getById<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="ancestor::table/name"/></xsl:call-template>(<xsl:apply-templates mode="CamposLlavesTabla" select="primary_key/field"/>); 
           <xsl:apply-templates mode="LlenarRequest" select="field"/>
...
<!-- Optiene todos los field de la tabla, por cada field genera un codigo para guardarlo en el REQUEST -->
<xsl:template mode="LlenarRequest" match="field">
           $_REQUEST["<xsl:value-of select="ancestor::table/name"/>__<xsl:value-of select="name"/>"] = $<xsl:value-of select="ancestor::table/name"/>_data[0]["<xsl:value-of select="name"/>"];</xsl:template> 
...
<!-- Optiene todos los field de la tabla que son Llaves, le antepone el simbolo '$' y le pospone el simbolo ',' --> 
