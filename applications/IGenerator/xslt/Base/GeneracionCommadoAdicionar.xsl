...
/**
 * Comando para adicionar un registro de la tabla: <xsl:value-of select="$tabla"/>
...
<xsl:template match="declaration">
	...
	function execute(){
		...
		if(<xsl:apply-templates mode="fields_not_null" select="ancestor-or-self::declaration"/>){
			...
			$message = $<xsl:value-of select="ancestor::table/name" />_manager->add<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="ancestor::table/name"/></xsl:call-template>(<xsl:apply-templates mode="CamposTabla" select="field"/>); 
...
<xsl:template mode="fields_null" match="field">
	<xsl:if test="null = 'null' and (type = 'int' or type = 'tinyint' or type = 'smallint' or type = 'mediumint' or type = 'bigint' or type = 'float' or type = 'float4' or type = 'float8' or type = 'double' or type = 'decimal' or type = 'int2' or type = 'int4' or type = 'int8' or type = 'number' or type = 'numeric' or type = 'NUMERIC' or type = 'CHAR' or type = 'VARCHAR' or type = 'INT')">
	...
	<xsl:if test="(type = 'varchar' or type = 'text' or type = 'numeric') ">
			$<xsl:value-of select="ancestor::table/name"/>__<xsl:value-of select="name"/> = $obj_datatype->formatString($<xsl:value-of select="ancestor::table/name"/>__<xsl:value-of select="name"/>);</xsl:if>	
</xsl:template>
...
<!-- Obtiene todos los field de la tabla, le antepone el simbolo '$' y le pospone el simbolo ',' --> 
...
<metaInformation>
<scenarios ><scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="..\..\..\..\..\..\..\My Documents\Tesis\Metadatos\dbpruebamysql2.xml" htmlbaseurl="" outputurl="" processortype="internal" profilemode="0" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" ><parameterValue name="tabla" value="'contrato_alquiler'"/></scenario></scenarios><MapperInfo srcSchemaPath="" srcSchemaRoot="" srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/>
</metaInformation>
-->