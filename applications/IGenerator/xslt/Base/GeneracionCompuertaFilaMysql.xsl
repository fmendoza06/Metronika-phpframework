 ...
 */
class <xsl:value-of select="$empresa"/><xsl:value-of select="$aplicacion"/>Mysql<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="name"/></xsl:call-template>
{
     /**
...
<xsl:template match="declaration">
	...
	* @author SpyroFrameWork
	*/
  	function <xsl:value-of select="$empresa"/><xsl:value-of select="$aplicacion"/>Mysql<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="ancestor::table/name"/></xsl:call-template>()
  	{
	...
	*/
  	function exist<xsl:call-template name="convertpropercase"><xsl:with-param name="toconvert" select="ancestor::table/name"/></xsl:call-template>(<xsl:apply-templates mode="LLAVES_PRIMARY_PARAMETROS" select="primary_key/field"/>)
  	{
    	...
    	return mysql_num_rows($this->consult);
  	}
