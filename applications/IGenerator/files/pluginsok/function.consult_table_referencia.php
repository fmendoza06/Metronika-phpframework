...
function smarty_function_consult_table_referencia($params, &$smarty)
{
 ...
 $gatewayReferencia = Application::getDataGateway($DataGateway);
  ...
  $html_tabla = '';
     ...
     if(isset($filter)) {
        CrearMecanismoBusqueda($html_tabla,$form_name,$command,$cantidad_paginas,$table_name,$filter);
     }
  ...
  $html_tabla .= "<tr>
                    <td colspan=4 class='detailedViewHeader1'>";


 $html_tabla .= "      <table border='0' align='left' width=100% class='dvtCellLabel'  colspan=3>
                           <tr height=50px width1=10%>
                 
                             <td width=10% height=32 class='detailedViewHeader' ><font class='small' size1='8' > <b>$title</b></font></div></td>
...
function CrearMecanismoBusqueda(&$html_tabla,$form_name,$command,$cantidad_paginas,$table_name,$filter)
{
    
    $html_tabla .= "<table border='0' class ='detailedViewTextBox' width=50%>";
    $html_tabla .= "<tr colspan=2 >";
    $html_tabla .=       "<td width=5% class='detailedViewHeader' >Filter</td>
                          <td width=5% class='detailedViewHeader' >Condiction</td>
                          <td width=5% class='detailedViewHeader' >Word(s) Key(s)</td>
 ...
 $html_tabla .=    "<td class='dvtCellLabel'><input class='detailedViewTextBox' type='text' name='key_word' id='key_wordid'";
    ...
    $html_tabla .=  "<td width=3%>";
                       $html_tabla .= "<input type='button' class='boton' value='Find' Onclick=\"disableButtons();action.value='".$command."';";
                       if($cantidad_paginas > 1 ){
...
function select()
{
  ...
  $html_result .= "<option value='='>Exact Frase</option>";
  $html_result .= "<option value='<>' selected>Diferent</option>";
  $html_result .= "<option value='_%'>Begin with</option>";
  $html_result .= "<option value='%_'>End With</option>";
  $html_result .= "<option value='%'>Content</option>";
...
function CrearEncabezadoTabla(&$html_tabla,$registros_tabla,$titulos){
    ...
    $html_tabla .= "<tr class='detailedViewHeader'>";
    $html_tabla .= "<td>";
    for($i=0;$i < count($m);$i++){
        $html_tabla .= "<th class='detailedViewHeader'>";
        $html_tabla.= $m[$i];
        $html_tabla .= "</td>";
    }
    $html_tabla .= "</tr>";
...
function CrearCuerpoTabla(&$html_tabla,$registros_tabla,$table_name,$llaves,$cambiar_valor,$cantidad_registros,$numero_pagina,$type,$command_showbyid,$form_name){
   ...
   for($i=(($numero_pagina-1)*$cantidad_registros);($i < $numero_pagina*$cantidad_registros )&&($i < count($registros_tabla));$i++){
      ...
      if($i%2 == 0)
         $valor_estilo ="dvtCellLabel";
      else
         $valor_estilo ="dvtCellInfo";
