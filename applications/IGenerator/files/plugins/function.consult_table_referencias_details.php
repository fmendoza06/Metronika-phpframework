 ...
 *           change_value = son los campos de la tabla que seran reemplazados por otros valores en otras tablas. (optional)
 ...
 *                          change_value="Codigo_Pais,pais,Codigo_Pais,Nombre_Pais"
...
function smarty_function_consult_table_referencias_details($params, &$smarty)
{
 ...
 $gatewayReferencia = Application::getDataGateway($DataGateway);
  ...
  if(isset($filter)) { 
	 ...
	 //consulta todos los registros
          ...
          $_REQUEST["key_word"] = strtoupper($_REQUEST["key_word"]);
   ...
   //$DataGatewayDetails // Tabla detalle
 ...
 if($cantidad_paginas > 1){
  ...
  if( (is_array($registros_tabla)) &&  (count($registros_tabla)>0)){
     ...
     if (!isset($change_value))
	   $change_value='';
     CrearCuerpoTabla($html_tabla,$registros_tabla,$table_name,$llaves,$change_value,$cantidad_registros,$numero_pagina,$typenew,$command_showbyid,$form_name,$DataGatewayDetails,$loadTableDetails,$paramsDetails,$detailsTitles,$detailKey);
  ...
  //Variables ocultas tabla Detalle
  CrearVariablesOcultas($html_tabla,$DataGatewayDetails,$detailKey);
...
* son los nombre de los campos de la tabla en la base de datos
*/
function CrearEncabezadoTablaDetails($registros_tabla,$titulos){
    require Application::getLanguageDirectory().'/'.Application::getLanguage()."/Tableheader.lan";
    //si la variable titulos esta setiada crea un vector con los titulos
    //sino crea un vector con los indices del vector $registros_tabla
     if(isset($titulos)){
...
*/
function CrearCuerpoTabla(&$html_tabla,$registros_tabla,$table_name,$llaves,$change_value,$cantidad_registros,$numero_pagina,$type,$command_showbyid,$form_name,$DataGatewayDetails,$loadTableDetails='',$paramsDetails='',$detailsTitles='',$detailKey){
   ...
   for($i=(($numero_pagina-1)*$cantidad_registros);($i < $numero_pagina*$cantidad_registros )&&($i < count($registros_tabla));$i++)
   {
      ...
      if(isset($change_value) ){
	     if($change_value !='' )  
            CambiarValorTabla($registros_tabla,$change_value,$i);
      }
       ...
       //obtener una fila completa de la tabla de la base de datos.
        ...
        //Busco la cantidad de registros detalles
         $gatewayReferencia = Application::getDataGateway($DataGatewayDetails);
         $registros = call_user_func(array($gatewayReferencia,"$loadTableDetails"."_cantidad"),$registros_tabla[$i]["$paramsDetails"]);
        ...
        for($j=0;$j < count($m);$j++){
           ...
           if($m[$j] != ""){
               if (($j == 0 || $j == 1) && ($type !='LOV') )
               {
                if ($j == 0 && $registros > 0)
                {
                 $html_tabla.= "           <a class='toolbar' href='#' onClick=\"visi('mainppal_".$registros_tabla[$i]["$paramsDetails"]."');\">";
                 $html_tabla.= '           <img title=Datos/Browse src="./web/images/menubar/go-down.png"></a>&nbsp';
                }
      ...
      $html_tabla .= "</tr>";
      if ($DataGatewayDetails!='')
      {

         if ($registros > 0)
         {
          $html_tabla.="<tr id='mainppal_".$registros_tabla[$i]["$paramsDetails"]."' style='display:none; position:relative'>";
          $html_tabla.=" <td colspan=14>";
          $html_tabla.="  <table class=adminlist>";
          $html_tabla.="   <tr>";
          $html_tabla.="     <th colspan=16>";
          $html_tabla.=buildToolBarDetail($DataGatewayDetails,$detailKey,$form_name);
          $html_tabla.="     </th>";
          $html_tabla.="   </tr>";
          $html_tabla.="   <tr>
                              <td colspan=16>
                                 <table class=adminlist>";
          $html_tabla.="            <tr colspan=16>";

          $html_tabla.=CrearEncabezadoTablaDetails($registros_tabla_details,$detailsTitles,'D');
          $html_tabla.='            </tr>';
          $html_tabla.=CreaRegistrosDetalle($DataGatewayDetails,$loadTableDetails,$registros_tabla[$i]["$paramsDetails"],$detailsTitles,$detailKey);
...
*/
function CreaRegistrosDetalle($DataGatewayDetails,$loadTableDetails,$paramsDetails,$detailsTitles,$detailKey='')
{

    $html_tabla='';
    $gatewayReferencia = Application::getDataGateway($DataGatewayDetails);
    if ($paramsDetails!='')
         $registros_tabla_details = call_user_func(array($gatewayReferencia,"$loadTableDetails"),$paramsDetails);
    else
    $registros_tabla_details = call_user_func(array($gatewayReferencia,$loadTableDetails));

    if ($registros_tabla_details!='' &&  is_array($registros_tabla_details))
    {
     for($i=0;$i<count($registros_tabla_details);$i++)
     {
          if($i%2 == 0)
           $valor_estilo ="row0";
         else
        ...
        $html_tabla .= "<tr id='".ucfirst($DataGatewayDetails)."__".$registros_tabla_details[$i][$detailKey]."' style='display:block; position:relative' class='".$valor_estilo."'>";
        $data = array_values($registros_tabla_details[$i]);
        for($j=0;$j < count($data);$j++)
        {
          if ($j==0)
           $html_tabla.=CrearCheckboxButonDetail($DataGatewayDetails,$detailKey,$data[0]);
          $html_tabla.='     <td>';
          $html_tabla .= $data[$j];
          $html_tabla.='     </td>';
        }
...
function buildToolBarDetail($table,$key='',$form_name)
{       //print_r($form);
        //$formMaster='RecursoConsult';
        $key=$table.'__'.$key;
        $table=ucfirst($table);
        //$form_name="frm".$formMaster;
        $appname = Application::getAppId();
		$html_tabla='';
        $html_tabla.='<table class=adminlist>';
        $html_tabla.='   <tr>';
        $html_tabla.='        <th colspan1=30>'; 
        $html_tabla.= "           <a class='toolbar' href='#'  onClick='".$form_name.".action.value=\"".$appname."CmdDefault".$table."\";".$form_name.".submit();' >";
        $html_tabla.= '           <img title=Nuevo/New src="./web/images/menubar/new_details.png"></a>';
        $html_tabla.= "           <a class='toolbar' href='#' onClick='".$form_name.".action.value=\"".$appname."CmdShowById".$table."\";".$form_name.".submit();'>";
        $html_tabla.= '           <img title=Editar/Edit src="./web/images/menubar/edit_details.png"></a>';
        $html_tabla.= "           <a class='toolbar' href='#' onClick=\"popUpShowShowDeleteForm('".$table."','$key');\">";
        $html_tabla.= '           <img title=Borrar/Del src="./web/images/menubar/delete_details.png"></a>';
        $html_tabla.='       </th>';
        $html_tabla.='   </tr>';
        $html_tabla.='</table>';
        return $html_tabla;
...
function CrearCheckboxButonDetail($table_name,$keys,$value){ //print_r("Entre..".$table_name);
  $html_tabla="";
  $html_tabla .= "<td>";
  $html_tabla .= "<input type='checkbox'";
  $html_tabla .= " name='".$table_name."__keys' onClick=\"";

  $html_tabla .= $table_name."__".$keys.".value = '".$value."'";
