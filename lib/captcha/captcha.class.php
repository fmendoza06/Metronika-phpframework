<?php
	/*
		...
		Author: Jose Fernando Mendoza 3136271441
		Email: fmendoza06@hotmail.com
		Date: 19/04/2010
	...
	*/
	class Captcha {
		/* Variables */
		//var $imageDirectory = 'c:/uweb2/instaladores/xampplite/xampplite/htdocs/citrium/applications/auth/web/captcha'; // No forward slash - must be writable, chmod 777
		var $imageDirectory = '.../citrium/applications/auth/web/captcha'; // No forward slash - must be writable, chmod 777
		//var $imageURL = 'http://localhost/citrium/applications/auth/web/captcha'; // No forward slash.
        var $imageURL = '../auth/web/captcha'; // No forward slash.
        var $captchaLabel = 'Ingrese el C&oacute;digo de la Imagen'; // This is what is shown just above the captcha image.
		var $captchaBoxStyle = 'border: 0px solid #ccc; padding: 3px; margin: 3px; width: 210px; font-family: verdana; font-size: 11px;'; // This is the style for the captcah box.
		...
		var $imageBGColor = array(255,255,255); //array(176,226,255); // RBG (Light Blue)
				...
				die('There was an error generating the Captcha - Check your directory path and permissions.');
					...
					$captchaBox = '<table>
					                <tr>
					                  <td>  
                                                             <center>
                                                                <fieldset><div style="'.$this->captchaBoxStyle.'">
...
//
                    $captchaBox = '	
								...
								<img src="'.$this->imageURL.'/'.$imageName.'" width="'.$this->imageWidth.'" height="'.$this->imageHeight.'" style1="border: 1px solid #000; margin: 2px;" alt="CAPTCHA Image">	
		...
		function generateImage($key) {
			// Create the image.
			$imageName = 'CAPTCHA_'.time().'_'.rand(1,101).'_'.rand(201,9007).'.gif';
			...
			$background = imagecolorallocate($image, $this->imageBGColor[0], $this->imageBGColor[1], $this->imageBGColor[2]);
                        //imagecreatefromgif("bgcaptcha.gif");
			// Set the text colour.
			...
			imageline($image, 90,0,80,50, $lineColor);
                        */
          	imageline($image, 0,0,rand(0,120),20, $lineColor);
			imageline($image, 1,0,rand(0,121),21, $lineColor);
			imageline($image, 40,0,rand(0,80),50, $lineColor);
			imageline($image, 41,0,rand(0,81),51, $lineColor);
			imageline($image, 75,0,rand(0,81),51, $lineColor);
			imageline($image, 90,0,rand(0,80),50, $lineColor);
			imageline($image, 120,0,rand(0,80),50, $lineColor);
			imageline($image, 140,0,rand(0,80),50, $lineColor);	
			imageline($image, 140,0,rand(0,80),50, $lineColor);
			imageline($image, 140,0,rand(0,80),30, $lineColor);
			imageline($image, 140,0,rand(0,80),21, $lineColor);			
			imageline($image, 140,0,rand(0,80),20, $lineColor);			
            ...
            imagestring($image, 5, 0, 0, $formattedKey, $textColor);
			// Save the image.
		...
		function verify($userInput) {
		  ...
		  $cookieDataHash = $_SESSION["captcha"];
